...
pr2 <- draws_zoib_diff %>% 
  ...
  scale_x_continuous(labels = scales::percent_format(1), 
                     breaks = seq(-0.1, 0.05, by = 0.02)) #+
  #theme(panel.grid.major.x = element_line())

cowplot::plot_grid(pr1, pr2)

ggsave(plot = cowplot::plot_grid(pr1, pr2), 
       filename = "figures/res_prop_bet.png", width = 8, height = 8*0.4, dpi = 500)
ggsave(plot = cowplot::plot_grid(pr1, pr2), 
       filename = "figures/res_prop_bet.pdf", width = 8, height = 8*0.4)
...
Same plot with observed data overlaid:
...
ggsave(plot = cowplot::plot_grid(ph2, ph2_diff), 
       filename = "figures/res_bernoulli.pdf", width = 8, height = 8*0.4)
```

Results figure with observed means:
...
Our hypothesis is about the mean betting time, which we need to calculate from the model parameters log-mean m and log-SD sigma as mean = ndt + exp(m + sigma^2/2).
...
ph3a <- draws_slognorm %>% 
  ...
  #stat_histintervalh(breaks = 40) +
  stat_halfeye(.width = ERROR_WIDTH) +
  xlab("Betting Time (s)") + ylab(ylabel)
...
ggsave(plot = cowplot::plot_grid(ph3a, ph3a_diff), 
       filename = "figures/res_lognormal.pdf", width = 8, height = 8*0.4)
...
The omitted betting times larger than 120 seconds are not completely equally distributed across conditions:
...
Even though the number of such observations is very low and they are almost equally distributed across conditions, we can rerun the model including all observations.

But first, we can take a look at some descriptive statistics of the distribution including all observations:
...
tmp_model_filename <- "model_fits/model_slognormal_v2.rda"
if (file.exists(tmp_model_filename)) {
  load(tmp_model_filename)
} else {
  m_slognorm2 <- brm(bf(time ~ expt_cond + (1|ppt_id), 
                       sigma ~ expt_cond), 
                    data = times_use, 
                    family = shifted_lognormal(),
                    iter = 6000, warmup = 1000, chains = 10, 
  ...
  save(m_slognorm2, file = tmp_model_filename, compress = "xz")
}
...
The model also shows no obvious convergence problems:

```{r, fig.asp=2.0}
plot(m_slognorm2, 
     variable = get_variables(m_slognorm2)[c(1, 2, 3, 4, 5, 6, 7, 8)], 
     N = 8)
```
...
```{r, fig.width=6, out.width="100%"}
pp1 <- pp_check(m_slognorm2, type = "hist", binwidth = 1, ndraws = 11) +
  theme(legend.position = "none") +
  coord_cartesian(xlim = c(0, 120), ylim = c(0, 1700))
pp1
...
```{r, include=FALSE}
draws_times_m <- emmeans(m_slognorm2, "expt_cond") %>% 
  gather_emmeans_draws() %>% 
  rename(m = .value)
draws_times_s <- emmeans(m_slognorm2, "expt_cond", dpar = "sigma") %>% 
  gather_emmeans_draws() %>% 
  mutate(.value = exp(.value)) %>% 
  rename(sigma = .value)
draws_slognorm <- left_join(draws_times_m, draws_times_s) %>% 
  mutate(mean = exp(m + sigma^2/2))
draws_times_diff <- draws_slognorm %>% 
  compare_levels(mean, by = expt_cond) %>% 
  filter(expt_cond != COMPARISON_EXCLUDE) %>% 
  mutate(comparison_with = str_remove(expt_cond, " - No Message"))
...
The following table shows the predicted mean betting times which are similar to the observed ones and reproduce the ordering of conditions means and is very similar to the model with excluded observations:
...
pspin1 <- nb_draws %>% 
  ...
  stat_halfeye(.width = ERROR_WIDTH) +
  ylab(ylabel) + xlab("Number of Spins")
pspin2 <- draws_number_diff %>% 
  ggplot(aes(x = trunc_mean, y = comparison_with)) +
  geom_vline(xintercept = 0, color = "grey", size = 0.6) +
  stat_halfeye(.width = ERROR_WIDTH) +
  labs(x = "Difference in Number of Spins") + ylab("No-Message vs.") 
cowplot::plot_grid(pspin1, pspin2)
ggsave(plot = cowplot::plot_grid(pspin1, pspin2), 
       filename = "figures/res_negbinomial.pdf", width = 8, height = 8*0.4)
```
