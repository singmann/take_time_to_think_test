...
draws_zoib_diff <- draws_zoib %>% 
  ...
  filter(expt_cond != "Message 2 - Message 1") %>% 
  mutate(comparison_with = paste("Message", str_extract(expt_cond, "\\d")))
...
```{r}
draws_number_diff <- mnb1 %>% 
  emmeans("expt_cond") %>% 
  gather_emmeans_draws() %>% 
  mutate(mean = exp(.value)) %>% 
  compare_levels(mean, expt_cond) %>% 
  filter(expt_cond != "Message 2 - Message 1") %>% 
  mutate(comparison_with = paste("Message", str_extract(expt_cond, "\\d")))
draws_number_diff %>% 
  median_qi(mean)
...
pspin1 <- mnb1 %>% 
  ...
  mutate(mean = exp(.value)) %>% 
  ungroup %>% 
  mutate(exp_cond = factor(expt_cond, labels = cond_labels)) %>% 
  ggplot(aes(y = expt_cond, x = mean)) +
  stat_halfeye() +
